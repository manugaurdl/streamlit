...
st.session_state.image_path = os.path.join(data_dir, df.iloc[st.session_state.current_proxy_idx]["img_path"].split('datasets/')[-1])
...
def text_to_heatmap(text_feat):        
        ...
        if (torch.sigmoid(out_bce.detach().cpu())>0.5).item():
            st.markdown("##### Object Exists")
        else:
